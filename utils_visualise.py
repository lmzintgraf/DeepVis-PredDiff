...
def plot_results(x_test, x_test_im, sensitivity_map, prediction_difference, target_function, classnames, save_path):
    """
    Plot the results of the relevance estimation
    """
    imsize = x_test.shape  
    
    target_idx = np.argmax(target_function(x_test)[-1])
    ...
    plt.subplot(2, 2, 2)
    plt.imshow(sensitivity_map, cmap=cm.Greys_r, interpolation='nearest')
    plt.title('sensitivity map')
    frame = pylab.gca()
    frame.axes.get_xaxis().set_ticks([])
    frame.axes.get_yaxis().set_ticks([]) 
    plt.subplot(2, 2, 3)
    p = prediction_difference.reshape((imsize[1], imsize[2], -1))[:, :, target_idx]
    ...
    frame.axes.get_yaxis().set_ticks([]) 
    plt.subplot(2, 2, 4)
    plt.title('class: {}'.format(target_class))
